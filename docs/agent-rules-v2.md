# agent-rules v0.2（Phase A）定稿：分層邊界 + 精度補強

> **單一權威定稿**（Claude × Codex 多輪收斂後合併）。取代並封存先前的 `agent-rules-v2-claude.md` / `agent-rules-v2-codex.md`（已移入 `docs/_archive/`）；實作直接依此。
...
- `docs/design-log.md` 只記 narrative snapshot，不複製完整表。
...
- Codex 有 hooks、skills（`.agents/skills/SKILL.md`）、plugins、memories、rules；這些不是 `AGENTS.md` 本身的 instruction layer。
...
- **skills 寫法（防 source-ownership 誤寫）**：README **prose 用通用詞「skills / plugins」+ 一句「安裝/快取/載入位置屬 tool-specific，不由本系統管理」**。`.agents/skills` 這種平台具體**只保留在表 A（Codex 能力描述）**，README 內文**不要**寫成「skill source 放 `.agents/skills`」。

### 3. templates/project-agent-rules/（只指針，不複製表）
- `generate.md` append 一行：hook/runtime config/skill/memory 不由本系統生成；強制性規則若可程式檢查或事件觸發請標 hook candidate；放置原則見 `GENERATE.vendored.md` §0。
- `behavior.md`：維持「本專案例外」定位，只加一行指針，不放表。

### 4. docs/design-log.md
- 新增 §12：本輪摘要 + 驗證事實 + 與 Codex 各版收斂過程（借敘事結構；保留 canonical-per-table、R2、execpolicy 陷阱；排除個人 skills 路徑）。
...
- 不把 memory 當 repo source；不把 memory 更新列為 blocking step；**即使要更新 memory 也只寫 machine-local，絕不為此在 repo 新增 `memory/`**；不複製表 A/B 到多檔。
- 不改 generator 既有輸出路徑、manual 區塊、banner、agent-targeting 機制。

## 備案（fallback）
- **§0 過大**（違反「§0 精簡」）：改成 §0 只放分界線 + 表 B 連結到新的低位章節（如 §9），README 仍持表 A。
- **32 KiB 語意不同**：若實作時對官方發現是「單檔」而非「合併」，把 §2 用詞從「合併預算」改「單檔上限」，保守精簡原則不變。
- **R2 未授權**：本輪 ai-rules repo 先完成，R2 留 follow-up（輸出手動指令）。
- **memory 不可寫**：只保留 design-log 記錄，不阻塞完成。

## 執行排序
1. 改 `GENERATE.md`、`README.md`、project template、`docs/design-log.md`（§1–4）。
2. 檢查 canonical 表格紀律與 scope 邊界。
3. 視環境更新 machine-local memory（optional、非阻塞）。
4. R2 同步全域影本（需 approval；未核准則輸出手動指令）。
5. 跑驗證 checklist。
6. commit ai-rules → main（+ push ~/agent-rules，若 R2 已做）。
...
- [ ] 無個人 skills 路徑進入 spec；README prose 用「skills / plugins」+ tool-specific disclaimer，未寫成「skill source 放 `.agents/skills`」
- [ ] design-log §12 僅 narrative snapshot、**無完整表 A/B**；§7 狀態正確
- [ ] R2 全域影本已同步並 push（或已輸出手動指令、不阻塞 ai-rules 完成）
- [ ] 本機 memory 已更新且未進 repo（**未為更新而新增 repo `memory/`**）；或環境無 memory，略過
- [ ] 無需 build
