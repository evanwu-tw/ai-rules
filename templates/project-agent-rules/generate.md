...
- **本系統只生成 instruction + context**：hook / runtime config / skill / memory **不由本系統生成**。強制性規則（能被程式檢查或事件觸發）請標為 **hook candidate**、改用 hook，不要只寫成 markdown。放置原則見 `GENERATE.vendored.md` §0 的決策表。
